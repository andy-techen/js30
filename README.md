...
- `Math.random()`
  - Returns a pseudo-random float in the range `0` (inclusive) to `1` (exclusive)
  - Use `Math.round(Math.random() * (max - min) + min)` to get random int between `min` and `max`

- `e.isTrusted`
  - Returns `true` when the event was generated by a user action, and `false` when the event was created/modified by a script or dispatched via `EventTarget.dispatchEvent()`
